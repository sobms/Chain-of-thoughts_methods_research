...
if __name__ == '__main__':
    ...
    with open('./bloom_output_method2', 'w') as output_file:
        outputs = []
        for input in tqdm(input_list[:TASK_COUNT]):
            ...
            for i in trange(ITERATIONS_COUNT):
                tokenized_input = tokenizer(cot_prompt + 'Q: ' + input['question'] + '\nA: ', return_tensors="pt")[
                    "input_ids"].cuda()
                output = model.generate(tokenized_input, max_new_tokens=80, do_sample=True, top_p=0.2, stop=['Q:'])
                CoT_sample.append(tokenizer.decode(output[0]))
            outputs.append(CoT_sample)
